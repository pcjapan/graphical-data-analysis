...
pre <- read.delim("pretest.txt", header = TRUE, sep = "\t")
c_Pre <- subset(pre, Class=="C")
memory <- c_Pre[c(4:12)]
cognitive <- c_Pre[c(13:26)]
compensation <- c_Pre[c(27:32)]
metacog <- c_Pre[c(33:41)]
affective <- c_Pre[c(42:47)]
social <- c_Pre[c(48:53)]
memoryTotal <- rowSums(memory[,c(1:9)])
cognitiveTotal <- rowSums(cognitive[,c(1:14)])
compTotal <- rowSums(compensation[,c(1:6)])
metacogTotal <- rowSums(metacog[,c(1:9)])
affectTotal <- rowSums(affective[,c(1:6)])
socialTotal <- rowSums(social[,c(1:6)])
corr_Dat <- read.delim("correlation.txt", header = TRUE, sep = ",")
...
The example below is generated from the simulated dataset used earlier for the notched boxplots, showing the results of a robust (Yuen) between-group _t_-test with explanatory measure of effect size ξˆ, and means and outliers annotated on the graph. Note that the CIs listed here are different to those in the paper. The CIs reported here are for the effect size in the graph, while those in the paper are the CIs of the difference in means. Other options for the type of _t_-test used are possible.
...
The example below is generated from the simulated dataset used earlier for the notched boxplots, showing the results of a robust (Yuen) _t_-test with effect size ξˆ, and means and outliers annotated on the graph. 

```{r}
ggstatsplot::ggbetweenstats(sim4, 
                            x = "group", 
                            y = "response", 
                            outlier.tagging = TRUE, 
                            type = "robust"
